...
void main() {
  var file = _createTestFile('test.txt');
  ...
  test('client_test.TusClient()', () async {
    final client = MockTusClient(file);
  ...
  test('client_test.TusClient().metadata', () async {
    ...
    // expect(client.fingerprint, isNot("test.txt"));
    expect(client.fingerprint, endsWith('test.txt'));
    expect(client.uploadMetadata, matches(RegExp('filename dGVzdC50eHQ=')));
    expect(client.uploadMetadata, matches(RegExp('id c2FtcGxl')));
  });
  ...
  test('client_test.TusClient().metadata.filename', () async {
    ...
    expect(
      ...
      matches(RegExp('filename YW5vdGhlci1uYW1lLnR4dA==')),
    );
    expect(client.uploadMetadata, matches(RegExp('id c2FtcGxl')));
  });
  ...
  test('client_test.TusClient.create()', () async {
    ...
    expect(
      verify(
        client.httpClient?.post(url, headers: captureAnyNamed('headers')),
      ).captured.first,
  ...
  test('client_test.TusClient.create().no.scheme', () async {
    ...
    when(client.httpClient?.post(url, headers: anyNamed('headers'))).thenAnswer(
      (_) async => http.Response(
        ...
        201,
        headers: {
          'location': '//example.com/tus/1ae64b4f-bd7a-410b-893d-3614a4bd68a6',
        },
      ),
    );
      ...
      throwsA(
        predicate(
          (e) =>
              ...
              e.message == 'missing upload Uri in response for creating upload',
        ),
      ),
    );
  });

  test('client_test.TusClient.createUpload().failure.server.error', () async {
    final client = MockTusClient(file);
    ...
    await expectLater(
      ...
      throwsA(
        predicate(
          (e) =>
              e is ProtocolException &&
              e.message == 'unexpected status code (500) while creating upload',
        ),
      ),
    );
  });
  ...
  test('client_test.TusClient.resume().failure.finger.not.found', () async {
    final client = MockTusClient(file, store: TusMemoryStore());
  ...
  test('client_test.TusClient.upload()', () async {
    ...
    expect(
      verify(
        client.httpClient?.post(any, headers: captureAnyNamed('headers')),
      ).captured.first,
      contains('Tus-Resumable'),
    );
    expect(
      verify(
        client.httpClient?.head(any, headers: captureAnyNamed('headers')),
      ).captured.first,
      contains('Tus-Resumable'),
    );
    expect(
    ...
    await expectLater(
      ...
      throwsA(
        predicate(
          (e) =>
              e is ProtocolException &&
              e.message == 'unexpected status code (500) while resuming upload',
        ),
      ),
    );
  });
    ...
    await expectLater(
      ...
      throwsA(
        predicate(
          (e) =>
              e is ProtocolException &&
              e.message == 'unexpected status code (500) while uploading chunk',
        ),
      ),
    );
  });
...
XFile _createTestFile(String name) {
  final f = File(name)
    ..writeAsBytesSync(List.generate(100, (index) => index >= 50 ? 1 : 0));
  final xf = XFile(f.path);
  return xf;
}

Future<void> _clearTestFile(String name) async {
  final f = File(name);
  if (f.existsSync()) {
    f.deleteSync();
  }
